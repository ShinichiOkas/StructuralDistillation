...
MUTANTS = [
    ...
    ("judge 交差検証なしを黙って済ませる", "compose.py", "    if budget.crosscheck and qs.crosscheck is None:", "    if False:"),
    ("judge 同じモデルの読み手を黙って済ませる", "compose.py",
     "    if len(real) >= 2 and len({model_of(r) for r in real}) == 1:", "    if False:"),
    ("judge 自己交差検証を黙って済ませる", "compose.py",
     "    elif qs.crosscheck is not None and {model_of(v) for v in verifiers} == {model_of(planner)}:", "    elif False:"),
