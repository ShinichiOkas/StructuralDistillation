...
MUTANTS = [
    ...
    ("judge 交差検証なしを黙って済ませる", "compose.py",
     '    if qs.crosscheck is None and budget.crosscheck and qs.source != "stored":', "    if False:"),
    ("judge 自己交差検証を黙って済ませる", "compose.py",
     "    elif qs.crosscheck is not None and verifiers is not None and planner is not None \\",
     "    elif False and verifiers is not None and planner is not None \\"),
    ("judge 生成器と読み手が同じモデルでも黙る", "compose.py",
     "    if planner is not None and len(models) == 1 and model_of(planner) in models:", "    if False:"),
    ...
    ("judge 注意を問いを作った経路だけにする", "compose.py",
     ...
     "    notes += _note_models(qs, budget, readers, used_verifiers, used_planner) if used_planner else []"),
