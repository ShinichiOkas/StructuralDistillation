...
MUTANTS = [
    ...
    ("judge 既定の検証役を注意の対象にしない", "compose.py",
     ...
     "    resolved_verifiers = list(verifiers) if verifiers is not None else []"),
    ("judge 偽読み手だけの走行を黙って済ませる", "compose.py", "    if not real:\n", "    if False:\n"),
    ("judge 生成器と読み手が同じモデルでも黙る", "compose.py",
     "    if len(models) == 1 and same:", "    if False:"),
    ("judge 作っていない問いの生成器を見ない", "compose.py",
     "(qs.planner in models and any(model_of(r) == qs.planner for r in real))", "False"),
    ("judge 名前だけの一致で同じモデルと言い切る", "compose.py",
     "            (qs.planner in models and any(model_of(r) == qs.planner for r in real)))",
     "            (qs.planner in {r.name for r in real}))"),
    ("judge 再利用のとき生成器の名前を告げない", "compose.py",
     "    elif len(models) == 1 and not generated:", "    elif False:"),
