...
MUTANTS = [
    ...
    ("l3 無効率の分母（回答ベース → 軸×標本）", "l3.py", "    n_answers = 2 * samples * n", "    n_answers = samples * n"),
    ...
    ("l3 s+r=0 を ρ に任せる", "l3.py", "    if s + r == 0:\n        return Label.NO_EVIDENCE, Reason.NO_DEFINITE_AXIS\n", "\n"),
    ("l3 軸 0 本を本文に根拠が無いにする", "l3.py", "        return Label.INSTRUMENT_FAULT, Reason.NO_ACTIVE_AXES",
     "        return Label.NO_EVIDENCE, Reason.NO_ACTIVE_AXES"),
    ("l3 理由を付けない", "l3.py", "                   value=None, diagnostics=diag, axes=axes, reason=reason)",
     "                   value=None, diagnostics=diag, axes=axes)"),
    ("judge リトライの材料を返さない", "compose.py", "    if qs.active_ids:\n        return None", "    if True:\n        return None"),
    ("judge リトライの手を見分けない", "compose.py", '    if qs.source == "given":', "    if False:"),
    ("judge 保存庫ありでも replan と言う", "compose.py", "    elif store is not None:\n", "    elif False:\n"),
    ("judge キャッシュ越しの作り直しを告げない", "compose.py", "        if plan_from_cache:\n", "        if False:\n"),
    ("judge 外れた軸の数を見ない", "compose.py", "    if flagged and len(flagged) == len(qs.axes):", "    if flagged:"),
    ("judge 交差検証なしを黙って済ませる", "compose.py",
     "    if qs.crosscheck is None and budget.crosscheck:", "    if False:"),
    ("judge 自己交差検証を黙って済ませる", "compose.py",
     "        if {model_of(v) for v in verifiers} == {model_of(planner)}:", "        if False:"),
    ("judge 生成器が検証役を兼ねても黙る", "compose.py",
     "        elif any(v.name == planner.name for v in verifiers) and len(verifiers) == 2:", "        elif False:"),
    # 受入 3 回目 C-1 の退行そのものの形（生成は正しく解決され、注意にだけ利用側の引数が渡る。受入 4 回目 m-17）
    ("judge 注意にだけ既定の生成器を渡さない", "compose.py",
     "    notes += _note_models(qs, budget, readers, resolved_verifiers, resolved_planner,",
     "    notes += _note_models(qs, budget, readers, resolved_verifiers, planner,"),
    ("judge 注意にだけ既定の検証役を渡さない", "compose.py",
     "    notes += _note_models(qs, budget, readers, resolved_verifiers, resolved_planner,",
     "    notes += _note_models(qs, budget, readers, list(verifiers or []), resolved_planner,"),
    ("judge 検出項目だけの問いにも出所を言う", "compose.py",
     'and any(a.kind != "detection" for a in qs.axes)', "and True"),
    ("judge 再利用でも名前の一致で言い切る", "compose.py",
     "    if len(models) == 1 and generated and planner is not None and model_of(planner) in models:",
     "    if len(models) == 1 and (model_of(planner) in models if generated and planner is not None"
     " else qs.planner in models):"),
    ("judge 偽読み手だけの走行を黙って済ませる", "compose.py", "    if not real:\n", "    if False:\n"),
    ("judge 生成器と読み手が同じモデルでも黙る", "compose.py",
     "    if len(models) == 1 and generated and planner is not None and model_of(planner) in models:",
     "    if False:"),
    ("judge 再利用のとき生成器の名前を告げない", "compose.py",
     '    elif len(models) == 1 and not generated and any(a.kind != "detection" for a in qs.axes):',
     "    elif False:"),
    ("judge 名前が一致しても手がかりを添えない", "compose.py",
     '        hint = ("。読み手のモデル名と同じ名前なので、1 モデルで回っている可能性が高い" if qs.planner in models else "")',
     '        hint = ""'),
    ("l0 別名でもモデル名を返さない", "l0.py", 'return getattr(reader, "model", None) or reader.name', "return reader.name"),
    ("l3 読み手の失敗を根拠の捏造と同じ理由にする", "l3.py",
     "(Reason.READER_FAILED if error_rate >= t.iota else Reason.INVALID_EVIDENCE)", "Reason.INVALID_EVIDENCE"),
    ...
    ("judge 1 モデル運用の注意を出さない", "compose.py", "    notes += _note_models(", "    _unused = _note_models("),
    ("judge 注意を問いを作った経路だけにする", "compose.py",
     "                          generated=qs.source == \"generated\")",
     "                          generated=qs.source == \"generated\") if qs.source == \"generated\" else []"),
    ("judge 読み手 1 体を数えない", "compose.py", "    if len(real) == 1:\n", "    if False:\n"),
    ("judge 読み手のモデルの重複を見ない", "compose.py", "    elif len(real) >= 2 and len(models) == 1:\n", "    elif False:\n"),
