...
MUTANTS = [
    ...
    ("judge 自己交差検証を黙って済ませる", "compose.py",
     "    elif qs.crosscheck is not None and (", "    elif False and ("),
    ("judge 再利用した問いの自己交差検証を見落とす", "compose.py",
     "            else set(qs.crosscheck.verifiers) == {qs.planner}):", "            else False):"),
    ("judge 生成器と読み手が同じモデルでも黙る", "compose.py",
     "    if len(models) == 1 and same:", "    if False:"),
    ("judge 作っていない問いの生成器を名前でも見ない", "compose.py",
     "else (qs.planner in models | names)", "else False"),
