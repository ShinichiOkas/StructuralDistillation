...
- ⚠ **1 つのモデルで全部を回すと判定は歪みます。** 弱い 4B のモデル 1 つで生成器・読み手・検証役を兼ねた測定では、
  想定に合った題材が 7/18 でした（複数モデルの基準は 16/18）。壊れるのは読み手ではなく生成器で、命題に効かない軸ばかりが作られます。
  同じモデルを 2 体に見せても読み手間の差は出ません。**生成器だけでも強いモデルにする**のが最も効きます。
  1 モデルのときは判定に注意が付きます（`Judgment.notes`）
