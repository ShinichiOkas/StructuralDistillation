...
4. **テストは本ライブラリ単体で走ること。** 利用側を起動しないと測れなくなったら、
   それは癒着が始まった合図です
5. ⚠ **このライブラリは LLM 非依存です。接続先は増やしません。**
   同梱の Ollama アダプタは参照実装で、**LLM プロバイダへの対応は利用側の責務**です
   （`Reader` の口を実装して差してください）。判定の仕組みと、どのモデルにどう繋ぐかは別の関心事です
...
依存は Python 3.11 以上の標準ライブラリだけです。LLM への接続は同梱の Ollama アダプタ（参照実装）か、
**自前の読み手**（`Reader` の口を実装したもの）で差します。⚠ 接続先を増やすのは利用側の仕事です（上記「設計上の約束」5）。
...
- ⚠ **1 つのモデルで全部を回すと判定は歪みます。** 弱い 4B のモデル 1 つで生成器・読み手・検証役を兼ねた測定では、
  想定に合った題材が 7/18 でした（複数モデルの基準は 16/18）。**主に壊れるのは生成器です**: 同じ弱いモデルでも、
  強いモデルが作った軸を読ませれば 13〜15/18 まで戻りました（標本数で変わります。読み手を強くすると さらに +1〜+3 件）。
...
- 生成器が付けた向きは、検証役に交差検証させ、過半数が反対した軸を外します（既定で有効）。
  ...
  ⚠ 検証役が 2 体でその 1 体が生成器のときは、外すのに全員一致が要るので**生成器が自分の軸に拒否権を持ちます**。
  モデルが別々でも起きるので、判定に注意が付きます（`Judgment.notes`）。
