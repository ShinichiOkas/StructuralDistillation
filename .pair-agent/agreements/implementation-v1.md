...
| C2 L3 | `test_l3` 32 緑。**S0a: 測定 2 周目の 8 腕・主走行と反事実の全 465 行**を引き直し、p・w・s・r・n・札（走行時 κ=0.5）・軸ごとの d・d_samples・一致率・矛盾・無効・沈黙、主走行の率と標本ごとの p が全一致。S4（側の入れ替えで p' = 1 − p、300 通り）・S12（軸の複製で p・w・札・率が不変、300 通り） | `l3_chat --probe base21 --id m01`: 実読み手 2 体 p 0.50・割れる、偽読み手 2 種は矛盾率 1.00 で計器不良（記録の札と一致） | ⑥ 設計 §4.3 の「矛盾率が [0.5, 0.667) の行は無い」は誤り（mono の m17 に 1 行）。S0a は走行時の閾値で比べる |
| C3 L4 | `test_l4` 21 緑（K 等分は空撃ちの式・切れ目・ラベル・値なし・2,000 通りで型の外に出ない・不正な型は InputError） | `l4_chat --ordinal 5`: 0→1・0.2→2・0.5→3・1→5。切れ目 0.3 / 0.7: 0.29→低・0.3→中・0.7→高。`--label NO_EVIDENCE` → 値なし | ⑦ 型の検査 `check_output` と、札を見ない `type_value`（代表値用）を L4 に置いた |
| D 合成 | `test_judge` 21 緑（入口の検査 F1・F2・重複名・偽の生成器／PlanningFailed／偽読み手 3 種で S11／既定の生成器と検証役／問いの集合を渡すと L1 を飛ばす／S8 呼び出し数とキャッシュ／標本と並列／記録 → replay（閾値・型・生応答からの引き直し）／judge_sync／単読み手／Q6 コアの import が標準ライブラリだけ） | **適合検査 ① 21/21 題材一致（主走行＋反事実）・実 0・外れ 0・命中 1,941。② 交差検証あり 21/21 一致（外した・弱・非排他・外した後の p / w）・実 0・外れ 0・命中 2,289。** 陰性対照: 回答の版を変えると 21/21 不一致・外れ 3,840（検査が落ちうることを確認） | ⑧ `replay(reparse=True)` を足した。`judge` に `verifiers` の偽読み手を拒む検査を足した |

### 変異試験（緑が偽物でないか）

`tools/mutation_check.py`: 実装を 1 箇所ずつ壊した 23 通り。初回は **21 殺・2 生き残り**:
H3 の閾値を 0.6 → 0.99 にしても落ちない（テストが完全一致の言い換えしか見ていなかった）／judge が交差検証で外した軸も集約しても落ちない（外れる軸がある judge のテストが無かった）。
テストを 2 本足して **23 殺・0 生き残り**。
